...
class FTwoBodyDynamicsSolver final : public IOrbitalMechanicSolver
{
protected:
    ~FTwoBodyDynamicsSolver() = default;

public:
    FTwoBodyDynamicsSolver()
    {
    ...
    virtual void SimulationStep(double mu, double dt, FSpatialState& state) override
    {
        ...
        state.GravityAcceleration = GetGravitationPull(mu, state);

        state.Velocity += (state.GravityAcceleration + state.Acceleration) * dt;
    }
